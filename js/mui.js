...
var witnUrl = "http://j1en001.6655.la:11237/Witn/"; 

/**
 * 普通的打开窗口可以调用这个方法
 * @param {Object} id  由于普通的打开窗口  是不需要请求网络数据的  已经预加载好了  直接传id
 */
...
/**
 * 
 * @param {Object} id  表单的id
 * @param {Object} allownull  是否容许全部的值为空 no  yes
 */
function gainFormValue(id,allownull) {
	var result = "";
	var havenull = false;
	mui("#"+id+" input").each(function() {
		var name = this.name;
		var value = this.value;
		if(  allownull=="no" && value=="" ){
			this.focus();
			havenull = true;
		}
		var type = this.type;
		if( type=="password" ){
			result += name+":"+hex_md5(value)+",";
		}else{
			result += name+":"+value+",";
		}
	});
